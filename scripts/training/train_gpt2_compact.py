#!/usr/bin/env python3
"""Fine-tune GPT-2 small on the compact Phase 1 dataset.

Parallel to scripts/training/train_gpt2.py. Differs only in defaults:
trains against dataset/processed/phase1_compact_hf, where each target
is the 22-hex-char variable region of the binary plus <END_BINARY>.
The constant ELF wrapper is reconstructed at generation time by
scripts/generation/generate_gpt2_compact.py.
"""
...
def main():
    p = argparse.ArgumentParser(description="Train GPT-2 small on compact Phase 1 dataset")
    ...
    if args.run_name:
        ...
        out = (
            "./models/checkpoints/gpt2-small-compact-test"
            if args.test
            else "./models/checkpoints/gpt2-small-compact"
        )
